...
class FileReceiver(TCPClient):
    ...
    def handle_data(self, segment: Segment):
        if segment.sequence_number == self.expected_sequence_number and segment.is_valid():
            self.file_data += segment.data

            self.expected_sequence_number += 1

            # send ACK for the received segment
            ack_segment = Segment.ack_segment(0, segment.sequence_number + len(segment.data))
